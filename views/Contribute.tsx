...
import { Language, SimPlan, Operator } from '../types';
...
export const Contribute: React.FC<ContributeProps> = ({ lang }) => {
  ...
  useEffect(() => {
    const ops = storageService.getOperators(selectedCountry);
    setAvailableOperators(ops);
    // Reset operator selection
    setFormData(prev => ({ ...prev, operator_id: ops.length > 0 ? ops[0].id : '' }));
    setIsNewOperator(ops.length === 0); // Auto-switch to new if none exist
  }, [selectedCountry]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isNewOperator) {
        ...
        storageService.addOperator(newOperator);
        finalOperatorId = newOpId;
        
        // Refresh local list so it shows up if they stay on page
    ...
    // Optionally keep the new operator selected, but reset the "New" toggle if desired. 
    // Here we keep the toggle as is for UX flow.
    if(isNewOperator) {
        setNewOperatorName('');
    ...
    <div className="max-w-2xl mx-auto bg-white rounded-xl shadow-lg overflow-hidden animate-fade-in">
      ...
      <div className="p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <button 
            type="submit" 
            className="w-full bg-green-600 text-white py-3 rounded-lg hover:bg-green-700 transition-colors font-bold flex justify-center items-center shadow-md"
          >
            <Save size={20} className="mr-2" /> {t.submit}
          </button>
        </form>
      </div>
    </div>
